...
| **P8a** | **Tauri commands v9 (sync_v9_cmds.rs) + HTTP client (sync_v9/client.rs) + HLC relocate** | Coexist v8 tạm thời. Move HLC helpers `sync_v9/hlc.rs`, update manual.rs/batch.rs call sites |
| **P8b** (pre-merge) | Migration v12 drop sync_state v8 columns + drop triggers | **Delete `src/commands/sync.rs` (1611 LOC) + `sync_client.rs` (378 LOC) + 12 v8 command registrations ở lib.rs.** Move `restart_app`, `machine_fingerprint`, `switch_db_to_user`, `sync_state_get/record_error` sang `commands/app_util.rs` hoặc inline vào sync_v9_cmds. Update `admin_view.rs` dùng `sync_v9::compress::zstd_decompress`. |

**Lý do split P8a/P8b:** Nếu delete v8 ngay ở P8, app không chạy được trong suốt P7 FE rewrite (v8 FE vẫn gọi v8 commands). Safe order: coexist → P7 FE switch sang v9 → P8b delete v8 khi không còn reference nào. Gate P8b: grep `commands::sync::|sync::sync_|sync_client::` → 0 matches.
